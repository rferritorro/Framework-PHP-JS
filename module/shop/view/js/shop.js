...
function select_all_cars(index = 0) {
    ...
    data = {limit: index,filtros: filter};
    $.when(
        ajaxPromise(friendlyURL('?page=shop&op=charge_cars'),'POST', 'JSON',data)
        .then(function(data) {
            var ciudades = [];
            $('div#map').remove();
        for (row in data) {
        ...
        if (token) {
            ...
            .then(function(car) {
                ...
                $('body').css('background-image','');
                    ...
                    ajaxPromise(friendlyURL('?page=shop&op=redirect'), 
                    'POST', 'JSON',info)
...
function load_filters() {
    ajaxPromise(friendlyURL('?page=shop&op=filters'),'GET','JSON')
    .then(function(car) {
        //filters #brand
        ...
        $(document).on('click','.select_brand',function () {
            ...
            if (brand == 0) {
                ...
                var data = {marca: brand};
                ajaxPromise(('?page=shop&op=filters'),'POST','JSON',data)
                    .then(function(model) {
                        $('<option></option>').attr({'id':'none','class':'option_model','value':'0'}).html("--").appendTo('#select_model');
                        for ( row in model) {
