function charge_items_type() {
    ajaxPromise(friendlyURL('?page=search&op=tipo'),'POST', 'JSON')
    .then(function(data) {
        for (row in data) {
...
function charge_items_brand(data=undefined) {
    ...
    ajaxPromise(friendlyURL('?page=search&op=category'),'POST', 'JSON',data)
    .then(function(data) {
        for (row in data) {
...
function increment_keyup_input() {
    $("#autocom").on("keyup", function () {
        ...
        ajaxPromise(friendlyURL('?page=search&op=autocomplete'),'POST', 'TEXT',data)
        .then(function(data) {
